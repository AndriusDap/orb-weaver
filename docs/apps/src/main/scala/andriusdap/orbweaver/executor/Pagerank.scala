...
class Pagerank(webgraph: String) {
  ...
  def seed(length: Int, seedFile: File, seedValue: Float): Array[Float] = {
    ...
    Source
      .fromFile(seedFile.jfile)
      .getLines()
      .map(_.toInt)
      .foreach(values(_) = seedValue)
    values
  }

  def output(pagerank: Array[Float], s: String): Unit = {
    File(s).writeAll(pagerank.zipWithIndex.map {
      case (rank, index) => s"$index $rank\n"
    }: _*)
  }

  case class PagerankResult(pagerank: Array[Float], convergence: Seq[Double])

  def pagerank(seedFile: File,
               passes: Int,
               dampening: Float,
               error: Double,
               seedValue: Float): PagerankResult = {
    val seedRank = seed(transposedGraph.length, seedFile, seedValue)

    val (convergence, pagerank) =
      (1 to passes).foldLeft(Seq[Double]() -> seedRank) {
        case ((diffs, currentRank), pass) =>
          if (!diffs.lastOption.exists(_ < error)) {
            val newValue =
              singlePass(transposedGraph, outDegree, currentRank, dampening)
  ...
  def singlePass(transposedGraph: Array[Array[Int]],
  ...
  def transpose(graph: BVGraph): Array[Array[Int]] = {
    require(
      graph.numNodes() < Int.MaxValue,
      s"Cannot handle more than maxint nodes, received ${graph.numNodes()}")

    val transposedGraph =
      Array.fill(graph.numNodes().toInt)(new ArrayBuffer[Int]())
