...
object App {
  ...
  def dumpLabelFiles(test: File,
    ...
    if (Seq(testLabelFile, trainLabelFile, validateLabelFile).exists(
          !_.exists)) {
      Seq(
        testLabelFile -> test,
  ...
  def main(args: Array[String]): Unit = {
    val (test, train, validate) =
      l("split files", () => splitFile(blacklist, whitelist))
    val (malicious, benign) = l("building seeds", () => getSeeds(train))
    val (maliciousPagerankFile,
         benignPagerankFile,
         maliciousPagerankConvergenceFile,
         benignPagerankConvergenceFile) =
      l("building pageranks", () => buildPageranks(malicious, benign))
    val (trainingFile, testingFile, validationFile) = l(
      "dump wabbit files",
      () =>
        dumpWabbitFiles(test,
                        train,
                        validate,
                        maliciousPagerankFile,
                        benignPagerankFile))
    val (trainingLabels, testingLabels, validationLabels) =
      l("dump label files", () => dumpLabelFiles(test, train, validate))
  }

  def dumpWabbitFiles(test: File,
                      train: File,
                      validate: File,
                      maliciousPagerankFile: File,
                      benignPagerankFile: File): (File, File, File) = {
  ...
  def extractFeatures(
      train: File,
      maliciousRanks: Map[String, Float],
      benignRanks: Map[String, Float]): Iterator[FeatureSet] = {
  ...
  case class FeatureSet(host: Vector[String],
                        query: Vector[String],
                        dots: Int,
                        specialSymbols: Int,
                        maliciousRank: Float,
  ...
  def readRankFile(benignPagerankFile: File): Map[String, Float] = {
    Source
      .fromFile(benignPagerankFile.jfile)
      .getLines()
      .map { s =>
        val split = s.split(" ")
  ...
  def buildPageranks(malicious: File, benign: File): (File, File, File, File) = {
    ...
    val maliciousPagerankConvergenceFile = File(
      tmpDir + "malicious_pagerank_convergence.txt")
    val benignPagerankConvergenceFile = File(
      tmpDir + "benign_pagerank_convergence.txt")

    if (Seq(maliciousPagerankFile,
            benignPagerankFile,
            maliciousPagerankConvergenceFile,
            benignPagerankConvergenceFile).exists(!_.exists)) {
      ...
      val maliciousPagerankConvergenceFileBuffer =
        maliciousPagerankConvergenceFile.bufferedWriter()
      val benignPagerankConvergenceFileBuffer =
        benignPagerankConvergenceFile.bufferedWriter()

      val pagerankAlgo =
        l("initializing pagerank", () => new Pagerank(hostgraph))
      val maliciousPagerank = l(
        s"malicious pagerank",
        () => pagerankAlgo.pagerank(malicious, 20, 0.95f, 0.1, 100))
      val benignPagerank = l(
        s"benign pagerank",
        () => pagerankAlgo.pagerank(benign, 50, 0.75f, 0.1, 1))
      ...
      maliciousPagerank.pagerank.zip(hosts).foreach {
        case (rank, host) =>
          maliciousPagerankFileBuffer.write(s"$host $rank\n")
      }

      benignPagerank.pagerank.zip(hosts).foreach {
        case (rank, host) =>
  ...
  def splitFile(blacklist: String, whitelist: String): (File, File, File) = {
    ...
    if (Seq(train, test, validate).exists(!_.exists)) {
      val blacklistLines = Source
        .fromFile(blacklist)
        .getLines()
        .map(s => Feature(s, Malicious))
        .toVector
      val whitelistLines = Source
        .fromFile(whitelist, "ISO-8859-1")
        .getLines()
        .map(s => Feature(s, Benign))
        .toVector
